...
const priceInputFilter = (element) => {
  ...
  let floatPart = inputValue.substring(
    inputValue.length - 2,
    inputValue.length
  );
    ...
    printable.insertAdjacentHTML(
      ...
      <div style="height: 32.3cm; width: 23cm; padding: 3rem 2rem 0 2rem; page-break-after: always">
        ...
        <div style="height: 85%; display: flex; flex-direction: column; justify-content: space-evenly; align-items: center">
          <div style="color: #000; font-size: 6rem; font-weight: bold; text-align: center">${products[
            i
          ].toUpperCase()}</div>
          <div style="color: #000; font-size: 10rem; font-weight: bold; text-align: center">R$ ${
            prices[i]
          }</div>
        </div>
      </div>
  ...
  for (let i = 0; i < products.length; i++) {
    printable.insertAdjacentHTML(
      ...
      <div style="height: 14.85cm; width: 21cm; border-bottom: 1px dashed #000; display: flex; flex-direction: column; justify-content: space-evenly; align-items: center">
        <div style="color: #000; font-size: 5rem; font-weight: bold; text-align: center">${products[
          i
        ].toUpperCase()}</div>
        <div style="color: #000; font-size: 8rem; font-weight: bold; text-align: center">R$ ${
          prices[i]
        }</div>
    `
    );
  ...
  for (let i = 0; i < products.length; i++) {
    printable.insertAdjacentHTML(
      ...
      <div class="product-paper" style="height: ${height}cm; width: 10.49cm; border: 1px dashed #000; display: flex; flex-direction: column; justify-content: space-evenly; align-items: center">
        <div style="color: #000; font-size: 1.8rem; font-weight: bold; text-align: center">${products[
          i
        ].toUpperCase()}</div>
        <div style="color: #000; font-size: 3rem; font-weight: bold; text-align: center">R$ ${
          prices[i]
        }</div>
      </div>
    `
...
const quarter = () => {
  ...
  for (let i = 0; i < products.length; i++) {
    printable.insertAdjacentHTML(
      'beforeend',
      `
      <div class="product-paper" style="height: ${height}cm; width: ${width}cm; border: 1px dashed #000; display: flex; flex-direction: column; justify-content: space-evenly; align-items: center">
        <div style="color: #000; font-size: 2.8rem; font-weight: bold; text-align: center">${products[
          i
        ].toUpperCase()}</div>
        <div style="color: #000; font-size: 5rem; font-weight: bold; text-align: center">R$ ${
          prices[i]
        }</div>
      </div>
    `
