...
btnAdd.onclick = () => {
  ...
  productsData.insertAdjacentHTML('beforeend', `
    <div id="productBlock${productIndex}">
  ...
  for (let i = 1; i <= productIndex; i++) {
    ...
    printable.insertAdjacentHTML('beforeend', `
      <div style="height: 30.639cm; width: 21cm; display: block;">
        <div style="display: flex; align-items: center; height: 50%; text-align: center; font-size: 2.5cm; font-weight: bold; font-family: Arial, sans-serif">${description}</div>
        <div style="margin: 0; display: flex; align-items: center; text-align: center; height: 50%; text-align: center; font-size: 5cm; font-weight: bold; font-family: Arial, sans-serif">${price}</div>
      </div>
    `);
  }
